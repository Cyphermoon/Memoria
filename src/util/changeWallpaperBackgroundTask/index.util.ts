...
import { setAndroidWallpaper } from "../wallpaper"
import {
	ActiveFolderAndItemResponse,
	getActiveFolderItemImageURL,
	getUserActiveFolder,
...
export async function setWallpaperFromActiveFolder(
	...
	const res =
		...
		(await setAndroidWallpaper(
			applyEffectToCloudinaryImage(folderItem?.folderItem?.image, folderItem.folderItem?.description)
		))
 ...
 */
export async function NonHeadlessAndroidWallpaperUpdateChange(
	isActive: boolean,
	folderId?: string,
	updateIdx: boolean = true,
	category: CollectionOptionTypes = "personal"
) {
	try {
		if (!isActive) {
